...
import { IDC_ARROW, IDC_HELP } from "./common";
...
const oPlaylistManager = function (name) {
  this.name = name;
  this.playlists = [];
...
registerCallback("on_mouse_lbtn_up", function on_mouse_lbtn_up(x, y) {
  ...
  if (cTouch.down) {
    ...
    if (Math.abs(cTouch.scroll_delta) > 13) {
      cTouch.multiplier = ((1000 - cTouch.t1.Time) / 20);
      cTouch.delta = Math.round((cTouch.scroll_delta) / 13);
